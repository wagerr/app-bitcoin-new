...
export class AppClient {
  ...
  // TODO: not tested
  async signMessage(message: Buffer, pathElements: number[]): Promise<Buffer> {
    const clientInterpreter = new ClientCommandInterpreter(() => {});
    ...
    const chunksRoot = new Merkle(
      chunks.map((m) => hashLeaf(m))
    ).getRoot();

    return this.makeRequest(BitcoinIns.SIGN_MESSAGE,
      Buffer.concat([
        pathElementsToBuffer(pathElements),
